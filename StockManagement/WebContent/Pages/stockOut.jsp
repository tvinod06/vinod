...
<body>
	<form id="stockOutForm" name="stockOutForm" action="stockOutSubmit"
		method="post">
			...
			<div class="col-sm-6">
				<div id="cnt_form">
					...
					<div class="form-group">
						<div>
							<select name="productName" id="productName"
								class="form-control mail-field" onchange="getCategory()">
					...
					<table
						...
						id="productList" class="container">
						<thead>
							<tr>
								<th>Product Name</th>
								<th>Category</th>
			...
			<div class="col-sm-6 ">
				...
				<div class="form-group">
					<select name="customerId" id="customerId"
						...
						<c:forEach var="customer" items="${cutomersList}">
							<option value="${customer.cutomerId}">${customer.customerName}</option>
						</c:forEach>
					</select>
				</div>
				<div class="form-group">
				...
				<div class="form-group">
						<div>
							<input type="text" name="totalStockOutAmount" id="totalStockOutAmount"
								class="form-control name-field" required="required"
								placeholder="Total Stock Amount" readonly="readonly">
						</div>
					</div>
				<div class="form-group">
					<div>
				...
				<div class="form-group">
					<div>
						<input type="file" name="file" class="form-control name-field"
							id="file" required="required" placeholder="Receipt Upload"
							onchange="uploadFile('stock Out')">
					</div>
				</div>
				<div class="form-group">
					<button type="button" class="btn btn-primary"
						onclick="submitStockOutForm()">Submit</button>
				</div>
				</div>
			</div>
		</div>
	...
	<script type="text/javascript">
		...
		function submitStockOutForm() {
			if (array[0] == undefined) {
				alert("Add Stock out Product details");
				return;
			}
		...
		function addTableRow() {
			...
			if (map.hasOwnProperty(c.value)) {
				...
				document.getElementById("totalStockOutAmount").value = +(document
						.getElementById("totalStockOutAmount").value)
						+ +(document.getElementById("totalAmount").value);
				...
				array.push([ c.value, cell1.innerHTML, cell2.innerHTML,
						cell3.innerHTML, cell4.innerHTML, cell5.innerHTML ]);
				map[c.value] = array;
			}
			reset();
		}
