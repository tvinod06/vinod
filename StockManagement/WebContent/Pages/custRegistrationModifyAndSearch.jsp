...
<body>
	...
	<form id="custModification" name="custModification" action="submitCustModify" method="post">
			...
			<div class="col-sm-6">
				<div id="cnt_form">
					...
					<div class="form-group">
						<select name="customerName" id="customerName" class="form-control mail-field"
							required="required" onchange="getCustomerDtl()">
							<option value="-1">Customer Name</option>
							<c:forEach var="cutomersList" items="${cutomersList}">
									<option value="${cutomersList.cutomerId}">${cutomersList.customerName}</option>
								</c:forEach>
							</select>
					</div>
					<div class="form-group">
...
function addCustomerProdDtls(prodId, prodName, category, scale){
	var table = document.getElementById("productList");
	var rowCount = table.rows.length;
	var row = table.insertRow(rowCount);
	var cell1 = row.insertCell(0);
	...
	cell2.innerHTML = category;
	cell3.innerHTML = scale;
	cell4.innerHTML = "<center><a href=\"#\" onclick=\"deleteRow(this)\"><img width=20px src=\"${del}\" /></a></center>";
	array.push([ prodId, cell1.innerHTML, cell2.innerHTML,	cell3.innerHTML, cell4.innerHTML ]);
	map[prodId] = array;
}
...
function custRegistrationSubmit(){
	...
	document.custModification.submit();
	}
}
...
function getCustomerDtl(){
	if(document.getElementById('customerName').value == '-1'){
		return;
	}
	var xmlhttp;
	if (window.XMLHttpRequest) {
	...
	var url = "getCustomerDtl?customerId="
	...
	xmlhttp.onreadystatechange = function() {
		if (xmlhttp.readyState == 4) {
			if (xmlhttp.status == 200) {
				var jsonData = JSON.parse(xmlhttp.responseText);
				document.getElementById('startDate').value = jsonData.fromDate;
				if(jsonData.toDate != undefined){
					document.getElementById('endDate').value = jsonData.toDate;
				}else{
					document.getElementById('endDate').value = '';
				}
				document.getElementById('frequency').value = jsonData.frequency;
				var table = document.getElementById("productList");
				for(var i=1; i < table.rows.length ;i++){
					document.getElementById('productList').deleteRow(i);
					i=i-1;
				}
				for (var i = 0; i < jsonData.custProdDtls.length; i++) {
					addCustomerProdDtls(jsonData.custProdDtls[i].productUniqueId,jsonData.custProdDtls[i].productName,jsonData.custProdDtls[i].category,jsonData.custProdDtls[i].scale);
				}
			} else {
				alert('Something is wrong !!');
			}
